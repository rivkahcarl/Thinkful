...
@when(u'I enter a meal cost of $50 and a 20% tip')
def step_impl(context):
    br = context.browser
    br.get('http://localhost:5000')
    meal_cost = br.find_element_by_name("meal_cost")
...
@then(u'I should see it calculated to $10 as the tip amount')
def step_impl(context):
    br = context.browser
    tip_amount = br.find_element_by_id('results')
    assert tip_amount == "10"
