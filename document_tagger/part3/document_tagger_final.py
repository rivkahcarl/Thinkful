...
#create functions 
#1) Iterate over and opening the files
...
#2) Compiling user supplied keywords into regular expressions
def keywordtoregex(keywords):
  searches = {}
  keywords = sys.argv[2:]
  for key in keywords:
    searches[key] = re.compile(r'\b' + kw + r'\b', re.IGNORECASE)
  for search in searches:
    print "\"{0}\": {1}".format(search, len(re.findall(searches[search], full_text)))



#3) counting keywords in a document 
#4) stripping out metadata (author, title, etc) frmo Project Gutenberg documents



#5) A main() function that calls the other functions, supplying them with the necessary user supplied arguemnts at run time
...
for doc in documents:
  if doc.endswith('.txt'):
    ...
    with open(doc_path, 'r') as f:
      ...
      print "***" * 25
      print "Here's the info for doc {}:".format(doc)
      ...
      print "***" * 25
      print "Here's the keyword info for doc {}:".format(doc)
      for search in searches:
        print "\"{0}\": {1}".format(search, len(re.findall(searches[search], full_text)))
